...
class %CLASS%(NodeInstance):
    ...
    def update_event(self, input_called=-1):
        ...
        if self.input(0): d["attributes"].append({"name": "BsrLifetime", "value": float(self.input(0))})
        if self.input(1): d["attributes"].append({"name": "EnableNonErpProtection", "value": bool(self.input(1))})
        if self.input(2): d["attributes"].append({"name": "BeaconGeneration", "value": bool(self.input(2))})
        if self.input(3): d["attributes"].append({"name": "EnableBeaconJitter", "value": bool(self.input(3))})
        if self.input(4): d["attributes"].append({"name": "BeaconJitter", "value": self.input(4)})
