...
class %CLASS%(NodeInstance):
    ...
    def update_event(self, input_called=-1):
        d = {
            'name': "ns3::AarfWifiManager",
            'attributes': []
        }

        if self.input(0): d["attributes"].append({"name": "MinSuccessThreshold", "value": int(self.input(0))})
        if self.input(1): d["attributes"].append({"name": "MinTimerThreshold", "value": int(self.input(1))})
        if self.input(2): d["attributes"].append({"name": "MaxSuccessThreshold", "value": int(self.input(2))})
        if self.input(3): d["attributes"].append({"name": "TimerK", "value": float(self.input(3))})
        if self.input(4): d["attributes"].append({"name": "SuccessK", "value": float(self.input(4))})
