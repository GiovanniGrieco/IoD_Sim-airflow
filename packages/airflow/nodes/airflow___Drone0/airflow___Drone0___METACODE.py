...
class %CLASS%(NodeInstance):
    ...
    def update_event(self, input_called=-1):
        ...
        battery = self.input(1) if self.input(1) is not None else self.default_battery
        mechanics = {
            'name': 'ns3::Drone',
            'attributes': self.input(2) if self.input(2) is not None else []
        }
        ...
        peripherals = self.input(5) if self.input(5) is not None else []
